...
public class WorldRenderer {
    ...
    private TextureRegion blockTexture, coalTexture, stoneTexture, woodTexture, meatTexture, cookedmeatTexture, berrypasteTexture, berryTexture, grassTexture, wallTexture, doorTexture, doorOpenTexture,
            stickTexture, pebbleTexture, bedTexture;
...
//    private TextureRegion bulletTexture,
    ...
    private TextureRegion floorTexture, tilledTexture, irrigationEmptyTexture, irrigationFullTexture, darkTexture;
...
//        this.cam.position.set(CAMERA_WIDTH/2, CAMERA_HEIGHT/2, 0);
    ...
    private void loadTextures() {
        ...
        bedTexture = itemAtlas.findRegion("bed");
        ...
        darkTexture = itemAtlas.findRegion("dark");
        tilledTexture = itemAtlas.findRegion("tilled");
        irrigationEmptyTexture = itemAtlas.findRegion("irrigation-empty");
        irrigationFullTexture = itemAtlas.findRegion("irrigation-full");
...
//            debugRenderer.rect(rect.x, rect.y, rect.width, rect.height);
    ...
    private void drawDark() {
        ...
        Player player = world.getBob();
        boolean holdingFire = false;
        if (player.getStrongHand() != null && player.getStrongHand() instanceof Magic && ((Magic) player.getStrongHand()).getProjectileType().equals(FIREBALL)) {
            holdingFire = true;
        }
        for (int i = 0; i < world.getLevel().getWidth(); i++) {
            for (int j = 0; j < world.getLevel().getHeight(); j++) {
                if(!(holdingFire && player.getShieldCircle().contains(new Vector2(i+0.5F, j+0.5F)))) {
                    spriteBatch.draw(darkTexture, i, j, Block.getSIZE(), Block.getSIZE());
                }
            }
        }
    }
...
//                spriteBatch.draw(new TextureRegion(itemAtlas.findRegion(((FillableBlock) block).isActive() ? "fire-burning" : "fire")), block.getPosition().x, block.getPosition().y, block.getBounds().getBoundingRectangle().getWidth(), block.getBounds().getBoundingRectangle().getHeight());
            } else if (block.getBlockType() != null && block.getBlockType().equals(Block.BlockType.BED)) {
                Polygon polygon = block.getBounds();
                float rotation = polygon.getRotation();
                spriteBatch.draw(bedTexture, polygon.getX(), polygon.getY(), polygon.getOriginX(), polygon.getOriginY(), polygon.getBoundingRectangle().width, polygon.getBoundingRectangle().height, 1F, 1F,
                        rotation == 90 || rotation == 270 ? rotation - 90 : rotation);
        ...
        for (FloorPad floorPad : world.getLevel().getFloorPads()) {
            ...
            switch (floorPad.getType()) {
                ...
                case WATERFLOW:
                    ...
                    break;
                case IRRIGATION:
                    //todo draw the irrigation
                    straightPiece = floorPad.getRotation() == 0 || floorPad.getRotation() == 90 || floorPad.getRotation() == 180 || floorPad.getRotation() == 270;
                    StringBuilder name = new StringBuilder();
                    name.append(straightPiece ? "irrigation" : "irrigationBend")
                            .append(floorPad.isConnected() ? "-full" : "-empty");
                    floorFrame = new TextureRegion(itemAtlas.findRegion(name.toString()));
                    //floorFrame = irrigationEmptyTexture;
            }
            if (floorFrame != null) {
                if (floorPad.getType().equals(IRRIGATION)) {
                    spriteBatch.draw(floorFrame, floorPad.getPos().x, floorPad.getPos().y, Block.getSIZE()/2, Block.getSIZE()/2,
                            Block.getSIZE(), Block.getSIZE(), 1, 1, straightPiece ? floorPad.getRotation() : floorPad.getRotation() - 45);
    ...
    private void drawBob() {
        ...
        if (bob.getStrongHand() != null)  {
            ...
            if (o instanceof Item) {
                    ...
                    if (bob.getStrongHand() instanceof Item) {
                        ...
                        switch (item.getItemType()) {
                            ...
                            case SWORD:
                                ...
                                break;
                            case SHOVEL:
                                itemFrame = itemAtlas.findRegion("shovel");
                                itemWidth = bob.getWidth()*1.5F;
                                itemHeight = bob.getHeight()*1.5F;
                                itemRotation = 45;
                                itemRadius = bob.getWidth()/2;
                                break;
                            case HOE:
                                itemFrame = itemAtlas.findRegion("hoe");
                                itemWidth = bob.getWidth()*1.5F;
                                itemHeight = bob.getHeight()*1.5F;
                                itemRotation = 45;
                                itemRadius = bob.getWidth()/2;
                ...
                if (item.getItemType().equals(Item.ItemType.HOE) || item.getItemType().equals(Item.ItemType.SHOVEL)) {
                    Point gridRef = bob.getGridRef(bob.getRotation(), xPos, yPos);
                    Object ob = world.getLevel().getBlock(gridRef.x, gridRef.y);
                    ...
                    switch (item.getItemType()) {
                        case HOE:
                            toDraw = new TextureRegion(itemAtlas.findRegion("tilled"));
                            break;
                        case SHOVEL:
                            straightPiece = item.getRotation() == 0 || item.getRotation() == 90 || item.getRotation() == 180 || item.getRotation() == 270;
                            toDraw = straightPiece ? new TextureRegion(itemAtlas.findRegion("irrigation-empty")) : new TextureRegion(itemAtlas.findRegion("irrigationBend-empty"));
                            break;
                    }
                    if (ob == null) {
                        Color color = spriteBatch.getColor();
                        ...
                        color.a = 0.3f;
                        spriteBatch.setColor(color);
                        spriteBatch.draw(new TextureRegion(toDraw), gridRef.x, gridRef.y, 0.5F, 0.5F, 1F, item.getSize(),
                                1F, 1F, straightPiece ? item.getRotation() : item.getRotation() - 45);
                        color.a = oldAlpha;
                        spriteBatch.setColor(color);
                    }
                    float itemWidth = bob.getWidth()/2;
                    float itemHeight = bob.getHeight()/2;
                    float itemRotation = 90;
                    float itemRadius = bob.getWidth()/2;
                }
            } else {
